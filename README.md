...
2) [Gender wise Bike rider breakdown](#2-Gender-wise-Bike-rider-breakdown)
3) [Bike Utilization](#3-Bike-Utilization)
4) [Starting and Ending Location](#4-Starting-and-Ending-Location)
5) [Checkout times for User](#5-Checkout-times-for-User)
6) [Checkout times by Gender](#6-Checkout-times-by-Gender)
7) [Trips per weekday per Hour](#7-Trips-per-weekday-per-Hour)
8) [Trips by gender and Weekday per hour](#8-Trips-by-gender-and-Weekday-per-hour)
9) [Trips by Weekday and gender](#9-Trips-by-Weekday-and-gender)
